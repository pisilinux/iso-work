...
def unoverlay(project, path, ignore_error=False):
    image_dir = project.image_dir()
    run('/bin/umount {image_dir}/{path}'.format(path=path, image_dir=image_dir), ignore_error)
...
def setup_efi(project):
    ...
    os.makedirs("%s/EFI/pisi" % iso_dir)
    for name in os.listdir(path):
...
def setup_isolinux(project):
    ...
    copy(os.path.join(image_dir, "usr/share/hwdata/pci.ids"), dest)
    ...
    if os.path.exists(lxdmconf_path):
        ...
        for line in open(lxdmconf_path, "r").readlines():
            if line.startswith("#autologin-user=") or line.startswith("autologin-user="):
                ...
                # if os.path.exists("%s/usr/bin/mate-session" % image_dir):
                #     lines.append("autologin-session=mate-session\n")
                # elif os.path.exists("%s/usr/bin/startxfce4" % image_dir):
                #     lines.append("autologin-session=xfce4\n")
            else:
                lines.append(line)
        open(lxdmconf_path, "w").write("".join(lines))
    else:
...
def setup_live_gdm(project):
    ...
    if os.path.exists(gdmconf_path):
        lines = []
        for line in open(gdmconf_path, "r").readlines():
            if line.startswith("[daemon]"):
                ...
                lines.append(line)
        open(gdmconf_path, "w").write("".join(lines))

        # dil için bu dosyadan veri alınıyor
        # /etc/locale.conf:
        # LANG=de_DE.UTF-8
        # LC_MESSAGES=en_US.UTF-8
    else:
        print("*** {} doesn't exist, setup_live_gdm() returned".format(
            gdmconf_path))
...
def squash_live_config_image(project):
    ...
    # overlay parameters
    # to = os.path.join(workdir, "overlay/to")
    upperdir = os.path.join(workdir, "overlay/liveconfig")
    oworkdir = os.path.join(workdir, "overlay/wlive")

    overlay(project, image_dir[1:], config_image_dir, upperdir, oworkdir)

    # yalı eksikse ve proje tipi install ise yalı ve xdm yüklenmeli
    if "yali" not in project.all_install_image_packages:
        packages = ["yali"]
        ...
        run('pisi --yes-all --ignore-comar \
            --ignore-file-conflicts -D"%s" it %s \
            ' % (config_image_dir, " ".join(packages)))
    ...
    if project.type == "install":
        ...
        run("mkdir %s/run/pisilinux" % image_dir)
...
#        image_file = project.image_file()
        ...
        # -------------------------------------------------------------------
        # if project.type == "install":
        # FIXME: Eğer hiçbir görüntü yöneticisi yoksa xdm paketi yüklenmeli!
        if "xdm" in project.all_install_image_packages:
            # FIXME: Do not hard code installer name
            dm_config = "DISPLAY_MANAGER=yali"
        ...
        # FIXME: Bukısm da liveconfig de olmalı
...
def load_grub_params(project, initcpio=False):
    ...
    try:
        ...
        if len(ver) > 0:
            ...
            ver = "0.0"  # FIXME: bu kısım tarih olmalı

        if project.display_manager() == "xdm":
            rescue_tr = rescue % {"version": ver, "res_trans": "Kurtarıcı"}
            rescue_eng = rescue % {"version": ver, "res_trans": "Rescue"}
        else:
            rescue_tr = ""
            rescue_eng = ""

        dosya = open("%s/grub.cfg" % image_dir, "w")
        dosya.write(grb % {'fonts': loadfonts, 'initcpio': initcpio,
            'version': ver, "rescue_tr": rescue_tr, "rescue_eng": rescue_eng})
    except:
